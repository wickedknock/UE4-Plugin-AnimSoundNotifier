...
FString UCustomAnimNotify::GetSceneNameFromAnimation(FString animName) {
	...
	//UE_LOG(LogTemp, Warning, TEXT("Notifier: %s"), *animName);
	...
	else if (animName.Contains("Sc01_")) {
		return "SC01";
	}
	else if (animName.Contains("Sc04_")) {
		return "SC04";
	}
	else if (animName.Contains("Sc05A_")) {
		return "SC05A";
	}
	else if (animName.Contains("Sc05B_")) {
		return "SC05B";
	}
	else {
		return "";
